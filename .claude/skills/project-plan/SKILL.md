...
### Phase 5: UI/UX 개선 (2026-02-24 완료)
- [x] 전체 디자인 테마 변경: doom-64 다크 → 정부/공공기관형 라이트
- [x] 폰트 변경: Inter → Arial + Noto Sans KR (variable 방식)
- [x] 버튼 스타일 통일: rounded-[4px], 배경색 있는 스타일 적용
- [x] 배지 스타일 변경: rounded-[2px], 무채색 zinc 계열
- [x] 헤더/사이드바 배경색: 옅은 베이지(`#FAF7F2`)
- [x] 금액 표시: 우측 정렬 + tabular-nums
- [x] 줄간격 타이트하게: line-height 1.4
- [x] 군수담당자 대시보드 전면 개편 (판매소별·업체별 현황 추가)
- [x] 피복판매소 대시보드 전면 개편 (KPI 6개, 미처리주문/재고부족 테이블)
- [x] 품목관리 좌측 사이드바: 베이지 배경 적용
- [x] 버튼 일괄 개선: 검색/추가/삭제/수정/규격 등 모든 버튼 배경색 적용
